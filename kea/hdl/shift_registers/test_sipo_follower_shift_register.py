...
def dut_wrapper_args_setup(parallel_data_bitwidth):
    ...
    return dut_wrapper_args, dut_wrapper_arg_types

@block
def sipo_follower_shift_register_wrapper(**dut_wrapper_args):
    ''' A wrapper around the sipo_follower_shift_register so that we have a
    clock for the tests.
    '''

    dut_args = copy.copy(dut_wrapper_args)
    del dut_args['clock']

    return_objects = []

    return_objects.append(sipo_follower_shift_register(**dut_args))

    return return_objects

class TestSipoFollowerShiftRegisterInterface(KeaTestCase):

    def setUp(self):
        parallel_data_bitwidth = 16
        self.dut_wrapper_args, _dut_wrapper_arg_types = (
            dut_wrapper_args_setup(parallel_data_bitwidth))

    def test_parallel_data_bitwidth_of_one(self):
        ''' The `sipo_follower_shift_register` should raise an error if
        `parallel_data` is less than 2 bits wide.
        '''
        dut_wrapper_args, _dut_wrapper_arg_types = dut_wrapper_args_setup(1)

        self.assertRaisesRegex(
            ValueError,
            ('sipo_follower_shift_register: parallel_data should be greater '
             'than 1 bit wide.'),
            sipo_follower_shift_register_wrapper,
            **dut_wrapper_args,
        )

class TestSipoFollowerShiftRegister(KeaTestCase):
    ...
    @block
    def end_tests(self, n_tests_to_run, **dut_wrapper_args):
    ...
    def data_stim(
        ...
        **dut_wrapper_args):
        ...
        clock = dut_wrapper_args['clock']
        serial_clock = dut_wrapper_args['serial_clock']
        shift_reg_nreset = dut_wrapper_args['shift_reg_nreset']
        parallel_out_nreset = dut_wrapper_args['parallel_out_nreset']
        serial_data = dut_wrapper_args['serial_data']
        latch = dut_wrapper_args['latch']
    ...
    @block
    def reset_stim(self, send_random_resets, **dut_wrapper_args):
    ...
    @block
    def stim_check(self, **dut_wrapper_args):
        ...
        parallel_out_nreset = dut_wrapper_args['parallel_out_nreset']
        latch = dut_wrapper_args['latch']
        parallel_data = dut_wrapper_args['parallel_data']
        ...
        return_objects.append(
            self.data_stim(
                ...
                parallel_stim_data, **dut_wrapper_args))
        ...
        def check():
            ...
            if not parallel_out_nreset:
                ...
                assert(parallel_data == 0)
                expected_parallel_data.next = 0

            elif latch and not shift_reg_nreset:
                # If latch has gone high at the same time as
                # shift_reg_nreset goes low then we should set the
                # parallel out to 0.
                assert(parallel_data == 0)
                expected_parallel_data.next = 0
                ...
                assert(parallel_data == pending_parallel_data)
                expected_parallel_data.next = pending_parallel_data

            else:
                # The parallel_data should hold its value
    ...
    def base_test(self, parallel_data_bitwidth, send_random_resets):
        ...
        def stimulate_check(**dut_wrapper_args):
            ...
            return_objects.append(self.stim_check(**dut_wrapper_args))
            return_objects.append(
                self.reset_stim(send_random_resets, **dut_wrapper_args))
            return_objects.append(self.end_tests(n_tests, **dut_wrapper_args))

            return return_objects

        dut_outputs, ref_outputs = self.cosimulate(
            cycles, sipo_follower_shift_register_wrapper,
            sipo_follower_shift_register_wrapper, dut_wrapper_args,
            dut_wrapper_arg_types,
    ...
    def test_data(self):
        ...
        `sipo_follower_shift_register` should shift the data in the shift
    ...
    def test_random_bitwidth(self):
        ''' The `sipo_follower_shift_register` should work correctly with any
    ...
    def test_bitwidth_of_two(self):
        ''' The `sipo_follower_shift_register` should work correctly with a
    ...
    def test_resets(self):
        ...
        `sipo_follower_shift_register` should set the every bit in the shift
        register low.

        On falling edges of `parallel_out_nreset` the
        `sipo_follower_shift_register` should set `parallel_data` to zero.
        '''
        self.base_test(
            parallel_data_bitwidth=16,
...
class TestSipoFollowerShiftRegisterVivadoVHDL(
    KeaVivadoVHDLTestCase, TestSipoFollowerShiftRegister):
    pass

class TestSipoFollowerShiftRegisterVivadoVerilog(
    KeaVivadoVerilogTestCase, TestSipoFollowerShiftRegister):
    pass
