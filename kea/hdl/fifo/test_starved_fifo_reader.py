...
class TestFifoReaderInterface(KeaTestCase):
    ...
    def test_mismatched_data_widths(self):
        ''' The `starved_fifo_reader` should raise an error if the `data_in`
        and `data_out` signals are not the same width.
        '''
    ...
    def test_zero_n_cycles_per_word(self):
        ''' The `starved_fifo_reader` should raise an error if
        `n_cycles_per_word` is zero.
        '''
    ...
    def test_large_buffer_n_words(self):
        ''' The `starved_fifo_reader` should raise an error if
        `buffer_n_words` is greater than 8.
        '''
        ...
        self.assertRaisesRegex(
            ...
            ('starved_fifo_reader: buffer_n_words is intended as a small '
             'buffer to overcome slight phase alignment discrepancies '
             'between the write and read clocks. If the buffer_n_words is '
             'too large for the FIFO then data will be lost. We limit '
             'buffer_n_words to 8 to protect against that data loss.'),
            starved_fifo_reader,
            **self.args,
        )
...
class TestFifoReader(KeaTestCase):
    ...
    def test_starved_fifo_reader(self):
        ''' The `starved_fifo_reader` should remain idle and wait until the
        ...
        passed the `starved_fifo_reader` should set `fifo_read_enable` high
        for one clock cycle every `n_cycles_per_word`. This reads one word out
        of the FIFO.

        ``buffer_n_cycles = buffer_n_words * n_cycles_per_word``

        The `starved_fifo_reader` should continue reading words out of the
        FIFO in this manner until `fifo_empty` goes high. It should then stop
        ...
        When `data_in_valid` is set high the `starved_fifo_reader` should
        forward the value on `data_in` to `data_out`.
        '''
        ...
        def test(
            ...
            return_objects.append(
                self.starved_fifo_reader_check(
                    clock, data_in, data_in_valid, fifo_empty,
        ...
        dut_outputs, ref_outputs = self.cosimulate(
            cycles, starved_fifo_reader, starved_fifo_reader, self.args,
    ...
    def test_random_data_width(self):
        ''' The `starved_fifo_reader` should be able to handle any arbitrary
        data width. Ie `data_in` and `data_out` can be any width as long as
        they the same width.
        '''
    ...
    def test_single_cycle_per_word(self):
        ''' The `starved_fifo_reader` should be able to handle an
        `n_cycles_per_word` of 1.
        '''
    ...
    def test_random_n_cycles_per_word(self):
        ''' The `starved_fifo_reader` should be able to handle any
        `n_cycles_per_word` which is greater than 0.
        '''
    ...
    def test_zero_buffer_n_words(self):
        ''' The `starved_fifo_reader` should be able to handle a
        `buffer_n_words` of 0.
        '''
    ...
    def test_random_buffer_n_words(self):
        ''' The `starved_fifo_reader` should be able to handle any
        `buffer_n_words` which is in the range:
    ...
    def test_fifo_empty(self):
        ...
        has passed the `starved_fifo_reader` should keep reading the FIFO
        until the `fifo_empty` signal goes high.
        '''
    ...
    def test_fifo_empty_zero_buffer_n_words(self):
        ''' If the `buffer_n_words` is 0, the `starved_fifo_reader` should
        start reading from the FIFO as soon as `fifo_empty` goes low.
        '''
