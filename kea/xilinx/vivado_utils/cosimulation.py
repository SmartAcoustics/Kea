...
def _vivado_generic_cosimulation(
    ...
    if kea.xilinx.vivado_utils.VIVADO_EXECUTABLE is None:
        raise EnvironmentError('Vivado executable not in path')

    if period is None:
        period = cosimulation.PERIOD
...
#            minimal_wrapper().convert(hdl=target_language, path=tmp_dir)
        ...
        if target_language == 'VHDL':
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                try:
                    ...
                    raise kea.xilinx.vivado_utils.KeaConversionError(
                        'The convertible top from kea.testing.myhdl failed '
                        'to convert with the following error:\n%s\n'
                        'Though this could be a problem with your code, it '
                        'could also mean there is a problem with the '
                        'way you set the test up. Are all the signals '
                        'defined correctly and the signal types set up '
                        'correctly (importantly, all the outputs are defined '
                        'as such)? Alternatively it could be a bug in '
                        'kea.testing.myhdl.' % str(e))
                    ...
                    #raise kea.xilinx.vivado_utils.KeaConversionError(
                    #    'The convertible top from kea.testing.myhdl failed '
                    #    'to convert with the following error: %s\n'
        ...
        vivado_process = subprocess.Popen(
            [kea.xilinx.vivado_utils.VIVADO_EXECUTABLE, '-nolog',
             '-nojournal', '-mode', 'batch', '-source',
             simulate_script_filename],
...
def vivado_vhdl_cosimulation(
    ...
    config_file='kea-testing.cfg', template_path_prefix='', vcd_name=None,
    time_units='ns'):
