...
'''A module implementing the Xilinx DSP slice, making use of the
VivadoIP block.
'''
...
# The two bits we need from kea.xilinx.vivado_utils in this example are
# VivadoIP and PortDirection
from kea.xilinx.vivado_utils import VivadoIP, PortDirection
...
# Opmode enumeration
N_DSP_OPMODES = 4
DSP_OPMODE_MULTIPLY = 0
DSP_OPMODE_MULTIPLY_ADD = 1
DSP_OPMODE_MULTIPLY_ACCUMULATE = 2
DSP_OPMODE_MULTIPLY_DECCUMULATE = 3 # deccumulate means to subtract from P
...
class VivadoDSPMacro(VivadoIP):
    ...
    def __init__(self):
        ...
        # block. It might be the case that, for example, dsp_macro
...
@block
def DSP(A, B, C, P, opmode, clock_enable, reset, clock):
    '''A MyHDL DSP block, using the encrypted IP when it is converted.
    '''
    ...
    check_intbv_signal(opmode, 'opmode', val_range=(0, N_DSP_OPMODES))
    ...
    opmode_register1 = Signal(intbv(val=0, min=0, max=N_DSP_OPMODES))
    opmode_register2 = Signal(intbv(val=0, min=0, max=N_DSP_OPMODES))
    ...
    @always_comb
    def set_opmode_X():
        if opmode_register2 == DSP_OPMODE_MULTIPLY:
            opmode_X.next = X_M
        elif opmode_register2 == DSP_OPMODE_MULTIPLY_ADD:
            opmode_X.next = X_M
        elif opmode_register2 == DSP_OPMODE_MULTIPLY_ACCUMULATE:
            opmode_X.next = X_M
        elif opmode_register2 == DSP_OPMODE_MULTIPLY_DECCUMULATE:
            opmode_X.next = X_M
        else:
            if __debug__:
    ...
    @always_comb
    def set_ALUMODE():
        if opmode_register2 == DSP_OPMODE_MULTIPLY_DECCUMULATE:
    ...
    DSP.verilog_code = dsp_macro.get_verilog_instance()
    DSP.vhdl_code = dsp_macro.get_vhdl_instance()

    return (dsp_block, opmode_pipeline,
