...
class TestReducingOrInterfaceSimulation(KeaTestCase):
    ...
    def test_invalid_output(self):
        ''' The `reducing_or` block should raise an error if the output is not a
        boolean signal.
        '''
...
@block
def reducing_or_wrapper(clock, output, input_signal):

    return reducing_or(output, input_signal)

class TestReducingOrSimulation(KeaTestCase):
    ...
    def test_random_width_input_signal(self):
        ''' The `reducing_or` block should be able to OR an input signal of any
        width.
        '''
        ...
        dut_outputs, ref_outputs = self.cosimulate(
            cycles, reducing_or_wrapper, reducing_or_wrapper,
            self.args, self.arg_types, custom_sources=[(test, (), self.args)])

        self.assertEqual(dut_outputs, ref_outputs)

    def test_one_bit_input_signal(self):
        ''' The `reducing_or` block should be able to handle an input signal
...
class TestReducingOrVivadoVhdlSimulation(
    KeaVivadoVHDLTestCase, TestReducingOrSimulation):
    pass

class TestReducingOrVivadoVerilogSimulation(
    KeaVivadoVerilogTestCase, TestReducingOrSimulation):
    pass
