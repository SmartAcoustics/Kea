...
class TestReducingAndInterface(KeaTestCase):
    ...
    def test_invalid_output(self):
        ''' The `reducing_and` block should raise an error if the output is
        not a boolean signal.
        '''
        ...
        self.assertRaisesRegex(
            ValueError,
            'reducing_and: output must be a boolean signal',
            reducing_and,
            **self.args,
        )

@block
def reducing_and_wrapper(clock, output, input_signal):

    return reducing_and(output, input_signal)

class TestReducingAnd(KeaTestCase):
    ...
    def test_random_input_bitwidths(self):
        ''' The `reducing_and` block should be able to AND an input signal of
        any width.
        '''
        ...
        dut_outputs, ref_outputs = self.cosimulate(
            cycles, reducing_and_wrapper, reducing_and_wrapper,
            self.args, self.arg_types, custom_sources=[(test, (), self.args)])

        self.assertEqual(dut_outputs, ref_outputs)

    def test_input_bitwidth_of_1(self):
        ''' The `reducing_and` block should be able to handle `input_signal`
    ...
    def test_input_boolean(self):
        ''' The `reducing_and` block should be able to handle an
        `input_signal` which is a boolean signals.
        '''
...
class TestReducingAndVivadoVhdl(
    KeaVivadoVHDLTestCase, TestReducingAnd):
    pass

class TestReducingAndVivadoVerilog(
    KeaVivadoVerilogTestCase, TestReducingAnd):
    pass
